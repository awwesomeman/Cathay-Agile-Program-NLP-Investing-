...
def clean_data(stock_id=('2330','台積電')):
    ...
    news_ = news[news.Tag.apply(lambda x: stock_id[1] in x)]
    ...
    vader_daily['sentiment'] = np.log((vader_daily['pos']+1)/(vader_daily['neg']+1))
    ...
    vader_daily = vader_daily.ewm(span=10).mean().dropna()
...
class MyBuySell(bt.observers.BuySell):
    plotlines = dict(
        buy=dict(markersize=10.0, color='#42f5dd',fillstyle='full'),#marker=r'$\Uparrow$' or '$\u21E7$'
        sell=dict(markersize=10.0, color='#9900cc',fillstyle='full') # marker=r'$\Downarrow$' or '$\u21E9$'
    )
# =========================================================    
# indicator
# =========================================================
class SentimentIndicator(bt.Indicator):
    lines = ('st','st_ma1','st_ma2','st_ma3','st_spread','st_spread_ma1','st_spread_ma2','st_mean')
    ...
    def next(self):
        # self.lines.st_mean[0] = np.mean(self.lines.st.get(size = self.counter))
        # self.lines.st_spread[0] = self.lines.st[0] - np.mean(self.lines.st.get(size = self.counter))
        # self.lines.st_spread_ma1[0] = np.mean(self.lines.st_spread.get(size = self.params.st_spread_ma1_period))
        # self.lines.st_spread_ma2[0] = np.mean(self.lines.st_spread.get(size = self.params.st_spread_ma2_period))      
...
class Strategy(bt.Strategy):
    ...
    def next(self):
        ...
        # ----------------------------------
        # print('=================')  
        # self.log('count',self.stind.st_spread[0])
        # print('-----------------')
        # self.log('rolling 0',self.stind.st[0])
        # print('-----------------')
        # self.log('origin',self.data.sentiment[0])
        # print('-----------------')
